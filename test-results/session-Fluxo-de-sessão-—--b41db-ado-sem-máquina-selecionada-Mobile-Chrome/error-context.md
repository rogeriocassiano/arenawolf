...
- Name: session.spec.ts >> Fluxo de sessão — usuário >> botão Gerar PIN desabilitado sem máquina selecionada
- Location: e2e/session.spec.ts:31:7
...
Locator: getByRole('button', { name: /PIN|Gerar|Usar PC/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /PIN|Gerar|Usar PC/i }).first()
